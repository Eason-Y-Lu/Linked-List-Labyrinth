...
namespace
{
    ...
    template <typename T>
    Grid<T> blankMaze(size_t numRows, size_t numCols, const T &value = T())
    {
    ...
    /* Row / column count. */
    template <typename T>
    size_t numRows(const Grid<T> &grid)
    {
        return grid.size();
    }
    template <typename T>
    size_t numCols(const Grid<T> &grid)
    {
    ...
    MazeCell *MazeCell::*randomFreePortOf(MazeCell *cell, mt19937 &generator)
    {
        ...
        if (cell->east == nullptr)
            ports.push_back(&MazeCell::east);
        if (cell->west == nullptr)
            ports.push_back(&MazeCell::west);
        if (cell->north == nullptr)
            ports.push_back(&MazeCell::north);
        if (cell->south == nullptr)
            ports.push_back(&MazeCell::south);
        if (ports.empty())
            return nullptr;
